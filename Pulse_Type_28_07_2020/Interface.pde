...
void keyPressed() {
  if (key == CODED) {
    if (keyCode == RIGHT) nextFont(); 
    else if (keyCode == TAB) serialPortFound = true;
    //loop();
  } else if (key == ' ') {
    if (!keyDown) {
      keyDown = true;
  ...
  } else if (key == 'p') initPDF();
  else if (key == 'a') serialPortFound = true;
  else if (key == 'f') switchFontBank(99);
  else if (key == 'r') switchFont();
  else if (key == 'q') endPDF(page);
}
